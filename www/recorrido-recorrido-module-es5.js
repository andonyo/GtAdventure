...
var RecorridoPageModule = /** @class */ (function () {
    function RecorridoPageModule() {
    }
...
var RecorridoPage = /** @class */ (function () {
    function RecorridoPage(alertController, router, barcodeScanner, db) {
    ...
    RecorridoPage.prototype.scanear = function () {
        var _this = this;
        this.barcodeScanner.scan().then(function (barcodeData) {
            var scan = JSON.parse(barcodeData.text);
            var id = scan.id.toString();
            var fin = scan.flag.toString();
            var p = scan.points.toString();
            var descip;
            var img;
            var puntos;
            var pUser = 0;
            _this.frase = scan.description;
            switch (id) {
                case "1":
            ...
            if (id == _this.id && fin == "1") {
                _this.puntuacion += parseInt(p);
            }
            else if (id == _this.id && fin == "2") {
                var fi = new Date(_this.fini);
                var ff = new Date();
                var resta = ff.getTime() - fi.getTime();
                var final = ff.getDate() + "/" + (ff.getMonth() + 1) + "/" + ff.getFullYear();
                var tiempo = resta / 1000 / 60;
                var time = tiempo.toFixed(2);
                puntos += _this.puntuacion;
                pUser = parseInt(_this.points) + puntos;
                _this.points = pUser.toString();
                var d = _this.db.CreatePremio(_this.correo, descip, img, puntos, time + " Minutos", final);
                var p_1 = _this.db.SumarPuntos(_this.correo, _this.points);
                var t = _this.db.GetCanTrofeos(_this.correo);
                localStorage.setItem("puntos", _this.points);
                _this.router.navigateByUrl('/felicidades');
            }
        });
    };
    RecorridoPage.prototype.presentAlert = function () {
    ...
    RecorridoPage.prototype.presentAlertConfirm = function () {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var alert;
            var _this = this;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                            ...
                            buttons: [
                                {
                                    ...
                                    text: 'Aceptar',
                                    handler: function () {
                                        _this.router.navigateByUrl('/triste');
                                    }
                                }
                            ]
                        })];
    ...
    RecorridoPage.prototype.ngOnInit = function () {
        console.log(this.id);
    };
    RecorridoPage.ctorParameters = function () { return [
...
//# sourceMappingURL=recorrido-recorrido-module-es5.js.map